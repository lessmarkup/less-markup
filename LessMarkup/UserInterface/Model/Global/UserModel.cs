...
namespace LessMarkup.UserInterface.Model.Global
{
    ...
    public class UserModel
    {
        public class Collection : IEditableModelCollection<UserModel>
        {
            ...
            {
                return
                    domainModel.GetCollection<User>().Where(u => !u.IsRemoved && u.SiteId.Value == SiteId).Select(u => u.UserId);
            }

            public IQueryable<UserModel> Read(IDomainModel domainModel, List<long> ids)
            {
                return domainModel.GetCollection<User>().Where(u => !u.IsRemoved && u.SiteId.Value == SiteId && ids.Contains(u.UserId)).Select(u => new UserModel
                    {
                        Email = u.Email,
                        Id = u.UserId,
                        Name = u.Name,
            ...
            public UserModel UpdateRecord(UserModel record, bool returnObject)
            {
                using (var domainModel = _domainModelProvider.CreateWithTransaction())
                {
                    var user = domainModel.GetCollection<User>().First(u => !u.IsRemoved && u.SiteId.Value == SiteId && u.UserId == record.Id);
            ...
            public bool DeleteRecords(IEnumerable<long> recordIds)
            {
                using (var domainModel = _domainModelProvider.CreateWithTransaction())
                {
                    ...
                    {
                        var user = domainModel.GetCollection<User>().First(u => !u.IsRemoved && u.SiteId == SiteId && u.UserId == userId);
