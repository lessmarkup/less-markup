...
namespace LessMarkup.UserInterface.NodeHandlers.Common
{
    public abstract class RecordListLinkNodeHandler<T> : RecordListNodeHandler<T> where T : class
    {
        ...
        protected void AddCellLink<TH>(string text, string link) where TH : IRecordNodeHandler
        {
        ...
        public override ChildHandlerSettings GetChildHandler(string path)
        {
            ...
            var handler = (IRecordNodeHandler) DependencyResolver.Resolve(cellLinkHandler.HandlerType);
